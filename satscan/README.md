...
drivers (seen on Zgemma) ignore the extension part of the hardware filter.
...
- **satscan-armhf** — ARMv7 + VFPv3-D16, **NEON explicitly disabled**: some
  STB SoCs (e.g. Zgemma H8) lack NEON and the default baseline SIGILLs; the
  no-NEON build runs on every ARM box we tried, NEON-capable ones included.
- **satscan-mipsel** — MIPS32 **r1**: older Broadcom MIPS (e.g. BCM7362 in
  Zgemma H3) SIGILL on r2 instructions. Also note MIPS encodes `ioctl`
...
| box | arch | dish setup |
|---|---|---|
| VU+ (2×DVB-S2X) | armv7l | universal LNB, no DiSEqC |
| VU+ FBC (8×) | armv7l | Unicable EN50494 (Sharp, auto-configured from settings) |
| Zgemma H8 | armv7l, no NEON | universal LNB |
| Zgemma H3 | mipsel r1 | universal LNB |
